...
public static class AdminSchemaShape
{
    private static readonly Regex CatalogTokens = new(
        """[eE]'(?:\\.|''|[^'\\])*'|'(?:''|[^'])*'|\$(?<tag>[A-Za-z_][A-Za-z_0-9]*|)\$[\s\S]*?\$\k<tag>\$|"(?:""|[^"])*"|[\p{L}\p{N}_$\u0080-\uFFFF]+""",
        RegexOptions.CultureInvariant);
    public const string Expected = "60d0d1701b68bf4aba755150142a1e8b51eab6119a83d6e0814295b23277b63c";

    internal static async Task VerifyAsync(NpgsqlConnection connection, NpgsqlTransaction tx, AdminConfiguration options, CancellationToken ct)
    {
        var actual = await FingerprintAsync(connection, tx, options, ct);
        if (actual != Expected) throw new InvalidOperationException("Схема Admin не соответствует известной миграции.");
    }

    public static async Task<string> FingerprintAsync(NpgsqlConnection connection, NpgsqlTransaction tx, AdminConfiguration options, CancellationToken ct)
    {
    ...
    internal static string NormalizeQualifiers(string value, AdminConfiguration options)
        => CatalogTokens.Replace(value, match =>
        {
            var end = match.Index + match.Length;
            if (end == value.Length || value[end] != '.') return match.Value;
            if (match.Value == options.Schema || match.Value == options.QuotedSchema) return "__ADM__";
