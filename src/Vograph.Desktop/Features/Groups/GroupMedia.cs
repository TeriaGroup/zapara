...
internal static class GroupMedia
{
    ...
    public static Task<ChatMessageResponse> Place(CommunityHttpClient api, string token, Guid conversationId, string kind, string name, byte[] bytes, Guid? replyTo, CancellationToken ct, int? durationMs = null)
    {
        ...
        return api.SendMediaAsync(token, conversationId, kind, string.IsNullOrWhiteSpace(clean) ? Label(kind) : clean, bytes, replyTo, ct, durationMs);
    }
