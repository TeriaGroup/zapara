...
public sealed class CommunityCliTests
{
    [Fact]
    public async Task Compiled_operator_baseline_rerun_and_redacted_failure()
    {
        ...
        var success = await Run(db, false, "communities", "db-migrate");
        ...
        Assert.Equal("{\"operation\":\"communities.db-migrate\",\"status\":\"failed\"}", failure.Output.Trim());
        Assert.DoesNotContain("CANARY", failure.Output);
        Console.WriteLine("CLI communities baseline=0 rerun=0 bad_args=2 invalid_config=5 redacted=true");
    }

    private static async Task<(int ExitCode, string Output)> Run(CommunityPostgresFixture db, bool invalid, params string[] arguments)
    {
        ...
        foreach (var key in start.Environment.Keys.Where(k => k.StartsWith("Accounts__", StringComparison.OrdinalIgnoreCase) ||
            k.StartsWith("Communities__", StringComparison.OrdinalIgnoreCase) || k.StartsWith("Sync__", StringComparison.OrdinalIgnoreCase) ||
        ...
        start.Environment["Communities__Schema"] = db.Schema;
