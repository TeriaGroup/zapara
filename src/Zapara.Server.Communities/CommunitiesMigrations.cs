...
public sealed class CommunitiesMigrations(AccountsDataSource dataSource, CommunitiesConfiguration configuration)
{
    ...
    public async Task EnsureAsync(CancellationToken ct = default)
    {
        ...
        var key = SHA256.HashData(Encoding.UTF8.GetBytes($"zapara.communities.migrations\n{connection.Database}\n{configuration.Schema}"));
        ...
        // Never initializes or repairs Accounts. Missing/unknown Accounts baseline fails before Communities DDL.
        ...
        if (objects == 0)
        {
            await Execute(Baseline().Replace("__COM__", configuration.QuotedSchema, StringComparison.Ordinal)
    ...
    private static InvalidOperationException Invalid() => new("Схема или подключение Communities не соответствует известной миграции.");
}
