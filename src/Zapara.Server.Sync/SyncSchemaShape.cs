...
internal static class SyncSchemaShape
{
    ...
    internal const string Expected = "5342dcfc5431f1c06bebd763390e31f8a10502582a47594a896dcaa26d7bd10e";
    internal static async Task VerifyAsync(NpgsqlConnection connection, NpgsqlTransaction tx, SyncConfiguration options, CancellationToken ct)
    {
        var actual = await FingerprintAsync(connection, tx, options, ct);
        if (actual != Expected) throw new InvalidOperationException("Схема Sync не соответствует известной миграции.");
    }
    internal static async Task<string> FingerprintAsync(NpgsqlConnection connection, NpgsqlTransaction tx, SyncConfiguration options, CancellationToken ct)
    {
    ...
    internal static string NormalizeQualifiers(string value, SyncConfiguration options)
    {
        return CatalogTokens.Replace(value, match =>
        {
            var end = match.Index + match.Length;
            if (end == value.Length || value[end] != '.') return match.Value;
            if (match.Value == options.Schema || match.Value == options.QuotedSchema) return "__SYNC__";
