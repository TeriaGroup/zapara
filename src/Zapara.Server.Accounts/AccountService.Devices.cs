...
{
    public Task<DevicesResponse> ListDevicesAsync(string accessToken, int limit = 20, string? cursor = null, CancellationToken ct = default, bool includeWeb = false)
    {
        ...
        {
            var position = AccountDeviceCursor.Parse(cursor, user.User.UserId, includeWeb);
            ...
            await using var command = db.Command($"""
                ...
                {(includeWeb ? "" : "AND platform IN ('windows','android')")}
            ...
            if (devices.Count > limit)
            {
                ...
                next = new AccountDeviceCursor(devices[^1].CreatedAt, devices[^1].FamilyId).Encode(user.User.UserId, includeWeb);
            }
