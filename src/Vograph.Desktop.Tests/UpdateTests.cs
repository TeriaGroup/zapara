...
public class UpdateTests : UiTest
{
    ...
    public async Task Check_Reports_Every_State()
    {
        ...
        source.Latest = new AutoUpdateService.UpdateInfo("windows-v2.1.12", "u", "z", "2026-09-01T00:00:00Z");
        Assert.False(await vm.CheckAsync());
        Assert.Equal(UpdateState.UpToDate, vm.State);
        Assert.Contains("windows-v2.1.12", vm.StatusText);
    ...
    public async Task Cleanup_Removes_Installed_And_Older_Downloads()
    {
        ...
        File.WriteAllBytes(Path.Combine(vm.UpdatesDir, "ZAPARA_windows-v2.1.12_win-x64.zip"), new byte[10]);      // this very version: installed
        File.WriteAllText(Path.Combine(vm.UpdatesDir, "ZAPARA_windows-v2.1.12_win-x64.zip.attempted"), "x");
        File.WriteAllBytes(Path.Combine(vm.UpdatesDir, "ZAPARA_windows-v1.2.2_win-x64.zip"), new byte[10]);      // older
        File.WriteAllBytes(Path.Combine(vm.UpdatesDir, "ZAPARA_windows-v2.1.12_win-x64.zip.part"), new byte[10]); // a torn download
